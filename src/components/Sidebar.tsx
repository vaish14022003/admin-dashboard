...
const Sidebar = () => {
    ...
    const adminId = useSelector((state: RootState) => state.auth.user?.adminId); // Safe optional access
    ...
    const handleLogout = async () => {
        try {
            ...
            await logoutAdmin(adminId); // API call with adminId
            dispatch(logout()); // Clear Redux auth state
            localStorage.removeItem('adminId'); // Clear localStorage if needed
